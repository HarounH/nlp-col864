...
class MemN2N(object):
    ...
    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size, answer_size,
        hops=3,
        max_grad_norm=40.0,
        nonlin=None,
    ...
    def _build_vars(self):
        with tf.variable_scope(self._name):
            ...
            # A = tf.concat(values=[ nil_word_slot, self._init([self._vocab_size-1, self._embedding_size]) ], axis=0)
            # C = tf.concat(values=[ nil_word_slot, self._init([self._answer_size-1, self._embedding_size]) ], axis=0)
            A = tf.concat([ nil_word_slot, self._init([self._vocab_size-1, self._embedding_size]) ], 0)
            C = tf.concat([ nil_word_slot, self._init([self._answer_size-1, self._embedding_size]) ], 0)
